...
import { SelectedItemContext } from '../BIM360Tree/Context';
				...
				setChildNodes(
					result.map((node) => {
						...
						switch (node.type) {
							...
							case 'items':
								return <TestTree key={node.id} {...node} icon={<InsertDriveFileIcon />} />;
							case 'versions':
								return (
									<TestTree
										key={node.id}
							...
							case 'unsupported':
								return <TestTree key={node.id} {...node} icon={<RemoveCircleIcon />} />;
							case 'none':
								return <TestTree key={node.id} {...node} icon={<ClearIcon />} disabled />;
							default:
								return <TestTree key={node.id} {...node} name="empty" />;
						}
					}),
				);
			});
