...
export const useData = (projectId, indexId, queryId) => {
	const [modelProperties, setModelProperties] = useState(null);

	const fetchModelProperties = async () => {
		const result = await axios.get(
			`${SERVER_URL}/api/forge/modelproperties/getModelPropertiesList?projectId=${projectId}&indexId=${indexId}&queryId=${queryId}`,
			{
